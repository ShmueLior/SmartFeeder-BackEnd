...
router.get("/:arduinoId/flags", async function (req, res, next) {
  ...
  if (dog == null) {
    return res
      .status(404)
      .send({ message: "No dog is registered for this device" });
  }
...
router.post("/:arduinoId/flags", async function (req, res, next) {
  ...
  try {
    ...
    if (req.body.bowlWeight != null) {
      //save statistic
      let howMuchFoodHeAteToDay =
        dog.howManyDropFoodToDay - req.body.bowlWeight;
      dog.bowlStatistic.push(
        new Object({ date: Date.now(), howMuchHeAte: howMuchFoodHeAteToDay })
      );
